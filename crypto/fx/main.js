...
var secret = "$think@360@FlowX";
var key = Crypto.enc.Utf8.parse(secret);
var iv = Crypto.enc.Utf8.parse(secret);
...
function decryptKey() {
	...
	try {
		document.querySelector("#enc-out").innerHTML = JSON.stringify(JSON.parse(decrypt(text)), undefined, 2);
	} catch (error) {
